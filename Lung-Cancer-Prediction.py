# Defining paths to the training, validation, and test datasets
...
# Defining paths to the specific classes within the dataset
...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, GlobalAveragePooling2D # type: ignore
...
# Initializing the image data generators for training and testing
...
# Setting up callbacks for learning rate reduction, early stopping, and model checkpointing
...
# Loading a pre-trained model (Xception) without the top layers and freeze its weights
pretrained_model = tf.keras.applications.Xception(weights='imagenet', include_top=False, input_shape=[*IMAGE_SIZE, 3])
pretrained_model.trainable = False

# Creating a new model with the pre-trained base and additional layers for classification
model = Sequential()
...
# Add this to your original training script (after model.fit())
model.save('trained_lung_cancer_model.h5')  # Saves to current directory
